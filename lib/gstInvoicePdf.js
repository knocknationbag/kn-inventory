...
async function loadPdfTools() {
  ...
  return { jsPDF, logo, stamp };
}

// Draws one GST Tax Invoice onto the current page of `doc` (adding pages as it needs them).
function drawGstInvoice(doc, { invoice, items, settings, logo, stamp }) {
  ...
  // ---- letterhead
  let y = 15;
  if (logo) doc.addImage(logo, "PNG", LEFT, 8, 20, 20);
  ...
  if (settings.phones) {
    doc.setFont("helvetica", "bold");
    doc.setFontSize(9);
    text(`Mob.: ${settings.phones}`, 105, y + 0.5, { align: "center" });
    y += 4;
  }
  ...
  if (bankRows) {
    ...
    yy += 4;
    doc.setFont("helvetica", "normal");
    const bank = [["Bank name", settings.bank_name], ["Account number", settings.bank_account], ["IFSC code", settings.bank_ifsc], ["Branch", settings.bank_branch]];
...
export async function buildGstInvoicePdf({ invoice, items, settings }) {
  const { jsPDF, logo, stamp } = await loadPdfTools();
  const doc = new jsPDF({ unit: "mm", format: "a4", compress: true });
  drawGstInvoice(doc, { invoice, items, settings, logo, stamp });
...
export async function buildGstReportPdf({ invoices, settings, from, to }) {
  ...
  for (const inv of invoices) {
    doc.addPage();
    drawGstInvoice(doc, { invoice: inv, items: inv.items, settings, logo, stamp });
  }
  return doc.output("blob");
}
