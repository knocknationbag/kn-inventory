...
// `qr` is a PNG data URL for the UPI "Scan & Pay" QR, or null when no UPI ID is configured.
function drawGstInvoice(doc, { invoice, items, settings, logoLeft, logoRight, stamp, qr }) {
  ...
  const footBoxMin = qr ? 44 : 18;
  const footerHeight = rows.length * 6 + 8 + wordsLines.length * 4 + 6 + notesLines.length * 4 + Math.max(declLines.length * 3.5 + bankRows * 3.8 + 6, footBoxMin) + 34;
  ...
  const footHeight = Math.max(yy - footTop, footBoxMin);
  doc.setDrawColor(...LINE);
  doc.rect(LEFT, footTop, leftW, footHeight);
  const qrBoxW = RIGHT - LEFT - leftW;
  const qrBoxX = LEFT + leftW;
  doc.rect(qrBoxX, footTop, qrBoxW, footHeight);
  doc.setTextColor(...NAVY);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(8.5);
  text("SCAN & PAY", qrBoxX + qrBoxW / 2, footTop + 5.5, { align: "center" });
  if (qr) {
    const qrSize = 24;
    doc.addImage(qr, "PNG", qrBoxX + qrBoxW / 2 - qrSize / 2, footTop + 8, qrSize, qrSize);
    doc.setFont("helvetica", "bold");
    doc.setFontSize(8);
    text(`Amount Payable: ${rs(invoice.grand_total)}`, qrBoxX + qrBoxW / 2, footTop + 8 + qrSize + 4, { align: "center" });
    doc.setFont("helvetica", "normal");
    doc.setFontSize(7);
    doc.setTextColor(...GRAY);
    text("Scan with any UPI app to pay", qrBoxX + qrBoxW / 2, footTop + 8 + qrSize + 8, { align: "center" });
  } else {
    doc.setFont("helvetica", "normal");
    doc.setFontSize(8.5);
    doc.setTextColor(...GRAY);
    text("UPI payment QR not configured", qrBoxX + qrBoxW / 2, footTop + footHeight / 2 + 4, { align: "center" });
  }
...
export async function buildGstInvoicePdf({ invoice, items, settings }) {
  const [{ jsPDF, logoLeft, logoRight, stamp }, qr] = await Promise.all([
    loadPdfTools(),
    buildUpiQrDataUrl({ upiId: settings.upi_id, payeeName: settings.upi_payee_name || settings.shop_name, amount: invoice.grand_total }),
  ]);
  const doc = new jsPDF({ unit: "mm", format: "a4", compress: true });
  drawGstInvoice(doc, { invoice, items, settings, logoLeft, logoRight, stamp, qr });
...
export async function buildGstReportPdf({ invoices, settings, from, to }) {
  ...
  for (const inv of invoices) {
    doc.addPage();
    const qr = await buildUpiQrDataUrl({ upiId: settings.upi_id, payeeName: settings.upi_payee_name || settings.shop_name, amount: inv.grand_total });
    drawGstInvoice(doc, { invoice: inv, items: inv.items, settings, logoLeft, logoRight, stamp, qr });
  }
  return doc.output("blob");
}
