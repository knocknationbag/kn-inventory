...
// Vercel caps a Function's request body (and response body) at a fixed 4.5MB, regardless of any
// Next.js config. These limits stay safely under that, with room for both files (preview) or the
// re-submitted JSON payload (confirm) in the same request, plus multipart/form overhead.
...
async function readFile(formData, key) {
  ...
  if (file.size > MAX_FILE_SIZE) return { error: "That file is too large (over 1.5 MB). Try exporting a shorter date range from the old app." };
...
async function buildPayloadFromFiles(formData) {
  ...
  if ((inv.text?.length ?? 0) + (bills.text?.length ?? 0) > MAX_PAYLOAD_SIZE) {
    return { error: "These files are too large to import together (over 3 MB combined). Try importing the inventory and bills backups one at a time." };
  }
...
export async function runLegacyImport(_prev, formData) {
  ...
  if (intent === "confirm") {
    ...
    if (typeof raw === "string" && raw) {
      if (raw.length > MAX_PAYLOAD_SIZE) return { error: "This import is too large to confirm in one go. Please import in smaller batches." };
      let payload;
      try {
