...
// "" is the default (no ?sort= in the URL), matching every other filter on this page. "manual" is the
// saved drag-and-drop order; it is only ever a view, so choosing any other sort never rewrites it.
export const SORT_OPTIONS = [
  { value: "manual", label: "Manual", column: "manual_sort_order", ascending: true },
  { value: "", label: "Oldest → Newest", column: "created_at", ascending: true },
  { value: "sku", label: "SKU (A-Z)", column: "sku", ascending: true },
...
export async function listProducts(supabase, { q, filter, category, supplier, sort, page }) {
  const sorting = SORT_OPTIONS.find((s) => s.value === sort) ?? SORT_OPTIONS.find((s) => s.value === "");
