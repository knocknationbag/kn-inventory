...
export const EXPORTS = {
  ...
  "movement:ledger": {
    ...
    load: async (s, f) =>
      (await getLedgerAll(s, f)).map((r) => ({ ...r, type: r.kind === "purchase" ? "Purchase" : r.kind === "sale" ? "Sale" : r.kind === "gst_sale" ? "Sale (GST)" : "Return", party: r.kind === "return" ? RETURN_REASON_LABEL[r.party] ?? r.party : r.party })),
  },
  "movement:date": {
  ...
  "sold:date": {
    filename: "sold-by-date",
    columns: [col("bill_type", "Bill type"), col("bill_no", "Bill no"), col("sku", "SKU"), col("product_name", "Product"), col("colour", "Colour"), col("size", "Size"), col("customer_name", "Customer"), col("customer_mobile", "Mobile"), col("quantity", "Qty"), col("rate", "Rate"), col("line_total", "Total")],
    load: async (s, f) => (await getSoldByDate(s, f.date)).rows.map((r) => ({ ...r, bill_type: r.bill_type === "gst" ? "GST invoice" : "Normal" })),
  },
  "low-stock": {
    filename: "low-stock",
