...
// `qrDataUrl` is generated by the caller (see lib/upiQr.js) so this component stays a plain sync render.
export default function GstInvoiceSheet({ invoice, items, settings, qrDataUrl }) {
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 grid gap-2 break-inside-avoid sm:grid-cols-[3fr_2fr] print:grid-cols-[3fr_2fr]">
        ...
        <section className="flex flex-col items-center justify-center gap-1 rounded border border-neutral-400 p-2 text-center text-[11px] sm:text-xs">
          <p className="font-bold uppercase tracking-wide">Scan &amp; Pay</p>
          {qrDataUrl ? (
            <>
              <img src={qrDataUrl} alt="UPI payment QR code" className="size-20 sm:size-24" />
              <p className="font-semibold">Amount Payable: {formatCurrency(invoice.grand_total)}</p>
              <p className="text-neutral-500">Scan with any UPI app to pay</p>
            </>
          ) : (
            <p className="text-neutral-500">UPI payment QR not configured</p>
          )}
        </section>
      </div>
